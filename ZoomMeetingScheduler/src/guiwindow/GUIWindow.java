...
public class GUIWindow extends JFrame implements ChangeListener, ActionListener, KeyListener {
    ...
    private ImageIcon dot=new ImageIcon(rsc.load("res/green.png"));
    ...
    private void initComponents() {
        ...
        this.setTitle("Zoom Meeting Scheduler");
        ...
        for (int i = 0; i < panel.length; i++) {
            ...
            for (int j = 0; j < panel.length; j++) {
                ...
                urlField[i][j].setToolTipText("Invite URL");
                        ...
                        for (int j = 0; j < panel.length; j++) {
                            if (timeField[i][j].isEnabled() && urlField[i][j].isEnabled()) {
                                ...
                                if (!(wd.isTimeValid(time) && wd.isUrlValid(url))) {
                                    isDataCorrect = false;
                                    lsc.frame.dispose();
                                    JOptionPane.showMessageDialog(null, "Invalid data detected! Please re-enter data properly.", "Error", JOptionPane.ERROR_MESSAGE);
                                    confirm.setEnabled(false);
                                    break;
                                }
                            }
